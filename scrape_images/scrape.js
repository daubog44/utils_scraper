...
async function get_url(query, maxRetries, timedout) {
  ...
  try {
    // vai a
    await nightmare.goto(`https://www.amazon.it/s?k=${query}&i=stripbooks`);
    ...
    if (data[0].itemUrl) {
      await nightmare.goto(data[0].itemUrl);
      ...
      $("#detailBullets_feature_div ul.a-unordered-list li").each((i, el) => {
        ...
        if (text) {
          textItemDetails.push(text);
        }
      });
      if (textItemDetails) data[0]["details"] = textItemDetails;
    }
    // fine
    await nightmare.end().catch((error) => {
      console.error("Search failed:", error);
